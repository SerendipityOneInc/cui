...
export class ClaudeProcessManager extends EventEmitter {
   ...
   */
  async startConversationNonBlocking(config: ConversationConfig & { resumedSessionId?: string }): Promise<{ streamingId: string }> {
    const isResume = !!config.resumedSessionId;
   ...
   */
  private async setInitialCommitHead(systemInit: SystemInitMessage): Promise<void> {
    if (!this.sessionInfoService || !this.fileSystemService) return;
