...
export class ConfigService {
    ...
    async loadConfig() {
        try {
            ...
            if (!fileConfig.server || JSON.stringify(merged.server) !== JSON.stringify(fileConfig.server))
                updated = true;
            if (!fileConfig.interface || JSON.stringify(merged.interface) !== JSON.stringify(fileConfig.interface))
                updated = true;
            if (!fileConfig.machine_id)
                updated = true;
    ...
    async updateConfig(updates) {
        ...
        const mergedInterface = updates.interface
            ? {
                ...
                notifications: updates.interface.notifications !== undefined
     ...
     */
    validateProvidedFields(partial) {
        // server
        if (partial.server) {
    ...
    assertInterfaceConfig(iface) {
        if (iface.colorScheme !== undefined && !['light', 'dark', 'system'].includes(iface.colorScheme)) {
    ...
    startWatching() {
        ...
        try {
            ...
            this.logger.warn('Failed to start file watcher for configuration', error);
        }
    }
    handleExternalChange() {
        try {
            ...
            const current = this.config || { ...DEFAULT_CONFIG, machine_id: '', authToken: '' };
            const merged = {
                ...DEFAULT_CONFIG,
                ...current,
                ...parsed,
                ...
                router: parsed.router !== undefined ? parsed.router : current.router,
                gemini: parsed.gemini !== undefined ? parsed.gemini : current.gemini,
            ...
            this.logger.error('Failed to handle external configuration change', error);
        }
    }
    /**
