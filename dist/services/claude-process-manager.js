...
export class ClaudeProcessManager extends EventEmitter {
    ...
    notificationService;
    routerService;
    conversationSyncService = new ConversationSyncService();
     ...
     */
    getConversationConfig(streamingId) {
     ...
     */
    async startConversationNonBlocking(config, options) {
    ...
    async waitForSystemInit(streamingId) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const messageHandler = ({ streamingId: msgStreamingId, message }) => {
                ...
                if (message?.type === 'system' && hookSubtypes.includes(messageSubtype)) {
        ...
        if (sessionId !== 'unknown') {
            this.historyReader.getConversationMetadata(sessionId)
                ...
                if (this.notificationService && metadata) {
                    this.notificationService.sendConversationEndNotification(streamingId, sessionId, metadata.summary).catch((error) => {
