    ...
    async readFile(requestedPath) {
        ...
        try {
            ...
            if (stats.size > this.maxFileSize) {
                throw new CUIError('FILE_TOO_LARGE', `File size (${stats.size} bytes) exceeds maximum allowed size (${this.maxFileSize} bytes)`, 400);
            }
            // Read file content
    ...
    async validateExecutable(executablePath) {
        ...
        try {
            ...
            try {
                ...
                throw new CUIError('NOT_EXECUTABLE', `File exists but is not executable: ${executablePath}`, 403);
            }
            ...
            throw new CUIError('EXECUTABLE_VALIDATION_FAILED', `Failed to validate executable: ${error}`, 500);
        }
    }
}
