...
export class CUIServer {
    ...
    setupRoutes() {
        ...
        // API routes
        this.app.use('/api/conversations', createConversationRoutes(this.processManager, this.historyReader, this.statusTracker, this.sessionInfoService, this.conversationStatusManager, this.toolMetricsService, this.streamManager));
    ...
    setupProcessManagerIntegration() {
        ...
        // Forward Claude messages to stream (including system init — let it flow through SSE)
        ...
        // Handle system-init-complete: perform post-init operations
        this.processManager.on('system-init-complete', async ({ streamingId, systemInit, config }) => {
            this.logger.debug('Received system-init-complete event', {
                streamingId,
            ...
            if (config.resumedSessionId) {
                try {
                    await this.sessionInfoService.updateSessionInfo(config.resumedSessionId, {
                        continuation_session_id: systemInit.session_id
                    });
                    this.logger.debug('Updated original session with continuation ID', {
                        originalSessionId: config.resumedSessionId,
                ...
                // Register the resumed session with conversation status manager
                try {
        ...
        // Handle system-init-failed: broadcast error event via SSE, close session
        this.processManager.on('system-init-failed', ({ streamingId, error }) => {
            this.logger.error('System init failed for session', error, { streamingId });
            const errorEvent = {
                type: 'error',
                error: error instanceof Error ? error.message : String(error),
