...
describe('Conversation Routes - Unified Start/Resume Endpoint', () => {
  ...
  describe('POST /api/conversations/start', () => {
    ...
    it('should handle resume with resumedSessionId (non-blocking)', async () => {
      ...
      // Post-init work (continuation ID, session registration) now happens
      // via system-init-complete event in cui-server.ts, not in the route
